    ...
    <div style={{ marginTop: '1rem' }}>
      ...
      <div style={{ marginBottom:'0.85rem' }}>
        ...
        <div style={{ display:'flex',alignItems:'center',gap:'0.5rem',background:'#08090d',border:`1px solid ${PB}`,borderRadius:8,padding:'0.45rem 0.7rem' }}>
          ...
          <button type="button" onClick={() => { navigator.clipboard.writeText(overlayUrl).catch(()=>{}); setCopied(true); notify('URL copiada!','success'); setTimeout(()=>setCopied(false),2000) }} style={{ flexShrink:0,padding:'0.22rem 0.6rem',background:copied?'rgba(34,197,94,0.15)':PBg,border:`1px solid ${copied?'rgba(34,197,94,0.3)':PB}`,borderRadius:5,color:copied?GR:P,fontSize:'0.7rem',fontWeight:700,cursor:'pointer' }}>
            {copied ? '✓ Copiado' : 'Copiar'}
          </button>
        </div>
      </div>
